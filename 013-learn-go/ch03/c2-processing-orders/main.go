...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	productAmountInStock := amountInStock(productID)
	totalPrice := calcPrice(productID, quantity)

	if productAmountInStock < quantity || accountBalance < totalPrice {
		return false, accountBalance
	}

	accountBalanceAfter := accountBalance - totalPrice

	return true, accountBalanceAfter
}
